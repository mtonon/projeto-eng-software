...
//        }
    ...
    private void carregaComboMotorista(){//SALVAR IDS
        itinerarioMotorista = new Motorista();
        arrayIdMotorista = new ArrayList<Integer>();
        ArrayList aux4=null;// = itinerarioMotorista.consultarTodosMotoristas();
        cboItinerarioMotorista.removeAllItems();
        cboItinerarioMotorista.addItem("Selecione");
        for (int i = 0; i < aux4.size(); i=i+2) {
        	arrayIdMotorista.add(Integer.parseInt((String) aux4.get(i))); //salvando ids Motorista
        	cboItinerarioMotorista.addItem(aux4.get(i+1));
        }
    	
    }
    
    private void carregaComboOnibus(){//SALVAR IDS
        itinerarioOnibus = new Onibus();
        arrayIdOnibus = new ArrayList<Integer>();
        ArrayList aux4 = null; //itinerarioOnibus.consultarTodosOnibus();
        cboItinerarioOnibus.removeAllItems();
        cboItinerarioOnibus.addItem("Selecione");
        for (int i = 0; i < aux4.size(); i=i+2) {
        	arrayIdOnibus.add(Integer.parseInt((String) aux4.get(i))); //salvando ids Onibus
        	cboItinerarioOnibus.addItem(aux4.get(i+1));
        }
    ...
    private Onibus itinerarioOnibus;
    private int flagMin=0;
    private Motorista itinerarioMotorista;
